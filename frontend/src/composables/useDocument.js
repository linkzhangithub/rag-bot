...
export function useDocument() {
  ...
  async function loadDocuments() {
    ...
    try {
      ...
      const invalidDocs = uploadedDocs.filter(
        (d) => !validDocNames.has(d.name),
      );

      // 如果有无效文档，清理 localStorage
      if (invalidDocs.length > 0) {
        console.log(
          `[INFO] 清理 ${invalidDocs.length} 个无效的缓存文档:`,
      ...
      console.log(
        `[INFO] 加载文档完成: ${mergedDocs.length} 个文档 (${baseDocs.length} 预置/API + ${uploadedDocs.length} 上传)`,
      );
    } catch (error) {
