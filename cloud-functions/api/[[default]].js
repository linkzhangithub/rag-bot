...
console.log(`[DEBUG] 文档目录: ${docsDir}`);
console.log(`[DEBUG] 文档目录存在: ${fs.existsSync(docsDir)}`);
...
async function initVectorSearcher() {
  ...
  try {
    ...
    console.log(
      `[INFO] 向量检索器初始化成功: ${vectorSearcher.getStats().totalVectors} 个文本块`,
    );
  } catch (error) {
...
app.get("/documents", (req, res) => {
  try {
    ...
    console.log(
      `[DEBUG] vectorSearcher 状态: ${vectorSearcher ? "已初始化" : "未初始化"}`,
    );

    if (vectorSearcher) {
      console.log(
        `[DEBUG] vectorSearcher.initialized: ${vectorSearcher.initialized}`,
      );
      console.log(
        `[DEBUG] vectorSearcher.vectors.length: ${vectorSearcher.vectors.length}`,
      );
    }
    ...
    if (
      vectorSearcher &&
      vectorSearcher.initialized &&
      vectorSearcher.vectors.length > 0
    ) {
      ...
      console.log(
        `[INFO] 已上传文档: ${Array.from(uploadedDocsMap.keys()).join(", ")}`,
      );
    } else {
...
app.post("/documents", async (req, res) => {
  try {
    ...
    if (vectors.length > 0) {
      ...
      console.log(
        `[INFO] 文档上传成功: ${fileName}, 共 ${vectors.length} 个文本块`,
      );
      console.log(
        `[INFO] 当前向量库总数: ${vectorSearcher.vectors.length} 个文本块`,
      );
...
app.delete("/documents/:name", async (req, res) => {
  try {
    ...
    console.log(
      `[DEBUG] vectorSearcher 状态: ${vectorSearcher ? "已创建" : "未创建"}`,
    );
  ...
  try {
    ...
    if (!apiKey) {
      const errorMsg = {
        type: "content",
        content:
          "错误：未配置 ZHIPU_API_KEY 环境变量。请在 EdgeOne 控制台配置环境变量。",
      };
    ...
    // 构建系统提示（使用实际检索到的文档）
    const docNames =
      usedDocuments.length > 0 ? usedDocuments.join("、") : "无可用文档";
